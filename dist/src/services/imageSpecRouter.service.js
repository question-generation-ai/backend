...
exports.ImageSpecRouter = void 0;
...
function wrapSvg(title, spec, body) {
...
function renderLabelList(spec, x = 520, y = 120) {
    return spec.labels
        .slice(0, 5)
        .map((label, index) => `<text x="${x}" y="${y + index * 24}" font-family="Arial, sans-serif" font-size="16" fill="#27272a">${escapeXml(label)}</text>`)
        .join('');
}
function has(spec, token) {
    return spec.elements.includes(token);
}
function hasAny(spec, tokens) {
    return tokens.some((token) => has(spec, token));
}
function renderForce(spec) {
    const arrows = [];
    if (has(spec, 'normal_force'))
        arrows.push(`<line x1="240" y1="220" x2="240" y2="130" stroke="#dc2626" stroke-width="5"/><path d="M240 130 l-12 18 M240 130 l12 18" stroke="#dc2626" stroke-width="5"/>`);
    if (has(spec, 'weight'))
        arrows.push(`<line x1="240" y1="220" x2="240" y2="320" stroke="#18181b" stroke-width="5"/><path d="M240 320 l-12 -18 M240 320 l12 -18" stroke="#18181b" stroke-width="5"/>`);
    if (has(spec, 'friction'))
        arrows.push(`<line x1="240" y1="220" x2="150" y2="220" stroke="#0f766e" stroke-width="5"/><path d="M150 220 l18 -12 M150 220 l18 12" stroke="#0f766e" stroke-width="5"/>`);
    if (has(spec, 'applied_force_horizontal'))
        arrows.push(`<line x1="240" y1="220" x2="340" y2="220" stroke="#2563eb" stroke-width="5"/><path d="M340 220 l-18 -12 M340 220 l-18 12" stroke="#2563eb" stroke-width="5"/>`);
    if (has(spec, 'applied_force_vertical'))
        arrows.push(`<line x1="240" y1="220" x2="240" y2="95" stroke="#7c3aed" stroke-width="5"/><path d="M240 95 l-12 18 M240 95 l12 18" stroke="#7c3aed" stroke-width="5"/>`);
    if (has(spec, 'applied_force_angled'))
        arrows.push(`<line x1="240" y1="220" x2="340" y2="160" stroke="#7c3aed" stroke-width="5"/><path d="M340 160 l-20 0 M340 160 l-10 18" stroke="#7c3aed" stroke-width="5"/>`);
    if (has(spec, 'tension'))
        arrows.push(`<line x1="240" y1="220" x2="340" y2="120" stroke="#ea580c" stroke-width="5"/><path d="M340 120 l-20 6 M340 120 l-6 20" stroke="#ea580c" stroke-width="5"/>`);
    ...
    return wrapSvg('Force Diagram', spec, `${plane}
...
function renderCircuit(spec) {
    return wrapSvg('Circuit Diagram', spec, `<g transform="translate(90 90)" stroke="#18181b" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
function renderCoordinateGraph(spec) {
    ...
    return wrapSvg('Coordinate Graph', spec, `<g transform="translate(70 80)">
...
function renderGeometry(spec) {
    ...
    return wrapSvg('Geometry Diagram', spec, `${shape}
...
function renderWave(spec) {
    return wrapSvg('Wave Diagram', spec, `<line x1="110" y1="240" x2="610" y2="240" stroke="#71717a" stroke-width="2"/>
...
function renderMolecular(spec) {
    return wrapSvg('Molecular Diagram', spec, `<line x1="220" y1="210" x2="320" y2="150" stroke="#71717a" stroke-width="${has(spec, 'bond_double') ? '8' : '6'}"/>
...
function renderCell(spec) {
    return wrapSvg('Cell Diagram', spec, `${has(spec, 'cell_wall') ? `<rect x="150" y="100" width="330" height="220" rx="26" fill="#dcfce7" stroke="#15803d" stroke-width="6"/>` : `<ellipse cx="315" cy="210" rx="180" ry="110" fill="#fee2e2" stroke="#e11d48" stroke-width="5"/>`}
...
function renderRayOptics(spec) {
    ...
    return wrapSvg('Ray Optics Diagram', spec, `<line x1="100" y1="210" x2="620" y2="210" stroke="#71717a" stroke-width="2"/>
    ${optic}
...
function renderGeneric(spec) {
    return wrapSvg('Question Figure', spec, `<rect x="110" y="100" width="500" height="220" rx="18" fill="#fafafa" stroke="#a1a1aa" stroke-width="2" stroke-dasharray="8 8"/>
    ${spec.elements
        .slice(0, 6)
        .map((element, index) => `<text x="140" y="${145 + index * 28}" font-family="Arial, sans-serif" font-size="18" fill="#3f3f46">${escapeXml(element)}</text>`)
        .join('')}
    ${renderLabelList(spec, 430, 145)}`);
}
class ImageSpecRouter {
    static isAllowedElement(type, element) {
        return imageSpec_1.ALLOWED_IMAGE_SPEC_ELEMENTS[type].includes(element);
    }
    static renderForceDiagram(spec) {
        return toDataUrl(renderForce(spec));
    }
    static renderCircuit(spec) {
        return toDataUrl(renderCircuit(spec));
    }
    static renderCoordinateGraph(spec) {
        return toDataUrl(renderCoordinateGraph(spec));
    }
    static renderGeometry(spec) {
        return toDataUrl(renderGeometry(spec));
    }
    static renderWave(spec) {
        return toDataUrl(renderWave(spec));
    }
    static renderMolecular(spec) {
        return toDataUrl(renderMolecular(spec));
    }
    static renderCellDiagram(spec) {
        return toDataUrl(renderCell(spec));
    }
    static renderRayOptics(spec) {
        return toDataUrl(renderRayOptics(spec));
    }
    static renderGeneric(spec) {
        return toDataUrl(renderGeneric(spec));
    }
    static shouldUseGenericFallback(spec) {
    ...
    static render(spec) {
        ...
        if (spec.type === 'force_diagram')
            return this.renderForceDiagram(spec);
        if (spec.type === 'circuit')
            return this.renderCircuit(spec);
        if (spec.type === 'coordinate_graph')
            return this.renderCoordinateGraph(spec);
        if (spec.type === 'geometry')
            return this.renderGeometry(spec);
        if (spec.type === 'wave')
            return this.renderWave(spec);
        if (spec.type === 'molecular')
            return this.renderMolecular(spec);
        if (spec.type === 'cell_diagram')
            return this.renderCellDiagram(spec);
        if (spec.type === 'ray_optics')
            return this.renderRayOptics(spec);
        return this.renderGeneric(spec);
    }
}
exports.ImageSpecRouter = ImageSpecRouter;
