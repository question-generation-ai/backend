...
class TemplateService {
    ...
    static async classifyImageRequirement(request) {
        const { questionContent, subject, complexity, preferredType } = request;
        // Explicit preferences
        if (preferredType === 'template')
            return 'template';
        if (preferredType === 'ai')
            return 'ai'; // Legacy, AI path is effectively removed but handling flag
        // Smart classification rules - default to template
        // Since AI generation is removed, we strongly prefer templates
        return 'template';
    }
    // Template-based Generation
    static async generateFromTemplate(templateId, parameters) {
        ...
        try {
            ...
            logger_1.default.error(`Template processing failed: ${error.message}`);
            throw error;
        }
    }
    // SVG Template Processing with Handlebars
    static async processSVGTemplate(svgContent, parameters) {
        try {
            // Compile template
            const template = handlebars_1.default.compile(svgContent);
            ...
            logger_1.default.error(`Handlebars compilation error: ${e.message}`);
    ...
    static async processCanvasTemplate(canvasConfig, parameters) {
        ...
        try {
            if (chartType === 'function_plot') {
                ...
                return await chart_service_1.ChartService.generateFunctionPlot(expression, range);
            }
            else {
                ...
                return await chart_service_1.ChartService.generateChart(type, data, title);
            }
        }
        catch (error) {
            logger_1.default.error(`Canvas template error: ${error.message}`);
            throw error;
        }
    }
    // Mermaid Template Processing
    static async processMermaidTemplate(mermaidContent, parameters) {
        try {
            // Compile template using Handlebars for dynamic node names/text
            const template = handlebars_1.default.compile(mermaidContent);
            return template(parameters);
        }
        catch (e) {
            logger_1.default.error(`Mermaid template error: ${e.message}`);
            return mermaidContent;
        }
    }
    // Layout/Exam Template Processing
    static async processLayoutTemplate(structure, parameters) {
        try {
            ...
            logger_1.default.error(`Layout template error: ${error.message}`);
            throw error;
        }
    }
    // Find suitable templates
    static async findSuitableTemplates(subject, keywords) {
