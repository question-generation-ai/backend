...
class TemplateService {
    ...
    static async classifyImageRequirement(request) {
        ...
        const requiresAI = aiRequiredKeywords.some(keyword => questionContent.toLowerCase().includes(keyword));
        if (requiresAI || complexity === 'complex') {
            return 'ai';
        }
        // Check if template can handle it
        const subjectKeywords = templateKeywords[subject] || [];
        const canUseTemplate = subjectKeywords.some(keyword => questionContent.toLowerCase().includes(keyword));
