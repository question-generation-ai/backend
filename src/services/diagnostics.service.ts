...
export interface SystemDiagnostics {
  ...
  systemHealth: {
    ...
    imageGeneration: 'healthy' | 'warning' | 'error';
    templates: 'healthy' | 'warning' | 'error';
    visualWorkflow: 'healthy' | 'warning' | 'error';
  };
  performance: {
    averageQuestionGenerationTime: number;
    averageImageGenerationTime: number;
...
export class DiagnosticsService {
   ...
   */
  static async runCompleteDiagnostics(): Promise<SystemDiagnostics> {
    const startTime = Date.now();
    logger.info('Starting complete system diagnostics...');
    ...
    try {
      ...
      logger.info(`System diagnostics completed in ${totalTime}ms`);
   ...
   */
  private static async testDatabaseHealth(diagnostics: SystemDiagnostics): Promise<void> {
    try {
   ...
   */
  private static async testTemplateSystem(diagnostics: SystemDiagnostics): Promise<void> {
    try {
      // Test template classification
      const testResult = await TemplateService.classifyImageRequirement({
      ...
      const templates = await TemplateService.findSuitableTemplates('mathematics', ['coordinate', 'graph']);
      ...
      logger.error(`Template system test failed: ${error}`);
   ...
   */
  private static async testImageGeneration(diagnostics: SystemDiagnostics): Promise<void> {
    try {
      const startTime = Date.now();
   ...
   */
  private static async testVisualWorkflow(diagnostics: SystemDiagnostics): Promise<void> {
    try {
      const startTime = Date.now();
      ...
      logger.error(`Visual workflow test failed: ${error}`);
   ...
   */
  private static async calculatePerformanceMetrics(diagnostics: SystemDiagnostics): Promise<void> {
    try {
      ...
      logger.warn(`Performance metrics calculation failed: ${error}`);
