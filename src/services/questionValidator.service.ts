...
export class QuestionValidatorService {
   ...
   */
  static async validate(question: any, type: string): Promise<ValidationResult> {
    ...
    logger.info(`Question validation: ${isValid ? 'PASSED' : 'FAILED'} (Score: ${score})`);
   ...
   */
  private static validateLength(question: any, type: string): ValidationResult['issues'] {
    const issues: ValidationResult['issues'] = [];
   ...
   */
  private static validateClarity(question: any): ValidationResult['issues'] {
    const issues: ValidationResult['issues'] = [];
   ...
   */
  private static validateOptions(question: any, type: string): ValidationResult['issues'] {
    const issues: ValidationResult['issues'] = [];
    ...
    const problematicOptions = options.filter((opt: string) =>
      /all of the above|none of the above|both a and b/i.test(opt.toLowerCase())
    );
    ...
    const optionsText = options.map((o: string) => o.toLowerCase()).join(' ');
   ...
   */
  private static validateAnswer(question: any, type: string): ValidationResult['issues'] {
    const issues: ValidationResult['issues'] = [];
   ...
   * Basic grammar check
   */
  private static validateGrammar(question: any): ValidationResult['issues'] {
    const issues: ValidationResult['issues'] = [];
   ...
   */
  private static calculateClarityScore(question: any): number {
    let score = 100;
    ...
    if (avgWordsPerSentence > 25) score -= 15; // Too complex
    if (avgWordsPerSentence < 5) score -= 10;  // Too simple/incomplete
    ...
    if (!hasQuestionMark && !hasQuestionWord) score -= 20;
   ...
   */
  private static estimateDifficulty(question: any): number {
    ...
    if (complexVerbs.some(v => text.includes(v))) difficulty = 4.0;
    else if (mediumVerbs.some(v => text.includes(v))) difficulty = 3.0;
    else if (simpleVerbs.some(v => text.includes(v))) difficulty = 1.5;

    // Adjust based on question length and complexity
    const words = text.split(/\s+/).length;
    if (words > 40) difficulty += 0.5;
    if (words < 10) difficulty -= 0.5;
   ...
   */
  private static assessPedagogicalValue(question: any): number {
    ...
    if (explanation.length > 50) score += 15;
    ...
    if (hasRealWorld) score += 15;

    // Check for common mistakes mentioned
    if (question.common_mistakes && question.common_mistakes.length > 0) score += 10;
   ...
   */
  private static checkTechnicalCorrectness(question: any): number {
    let score = 100;

    // Check for required fields
    if (!question.question) score -= 30;
    if (!question.correct_answer) score -= 30;
    // Explanation is optional

    // Check for proper formatting
    if (question.options && !Array.isArray(question.options)) score -= 10;
    if (question.difficulty_score && (question.difficulty_score < 1 || question.difficulty_score > 5)) score -= 10;
