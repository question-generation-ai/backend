...
export interface DifficultyProfile {
  ...
  conceptSpan: string;
  expectedTimeMin: string;
  distractorSimilarity: string;
...
export interface PromptGenerationPlan {
  batchSize: number;
  oversized: boolean;
  compactMode: boolean;
  reason: string;
...
const DIFFICULTY_PROFILES: Record<DifficultyLabel, DifficultyProfile> = {
  easy: {
    label: 'easy',
    uiLabel: 'Easy',
...
const DIFFICULTY_ALIASES: Record<string, DifficultyLabel> = {
  easy: 'easy',
  basic: 'easy',
  simple: 'easy',
  beginner: 'easy',
...
export const VAGUE_CONTROL_TERMS = [
  'easy',
  'medium',
  'hard',
  'simple',
...
export class PromptPolicyService {
  ...
  static buildDifficultyContract(label: string): string {
  ...
  static buildPromptFieldBudget(type: string, compactMode: boolean): string {
    const normalizedType = (type || '').toLowerCase();
    const base = [
  ...
  static findVagueTerms(text: string): string[] {
    const lower = (text || '').toLowerCase();
    return VAGUE_CONTROL_TERMS.filter((term) => lower.includes(term));
  }
}
