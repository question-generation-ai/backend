...
import { VisualContentAnalyzer, VisualContentRequirement, SubjectAnalysisResult } from './visualContentAnalyzer.service';
...
export interface VisualQuestionRequest {
  subject: string;
  chapter: string;
  difficulty: string;
  type: string;
...
export class VisualQuestionGenerator {
   ...
   */
  static async generateVisualQuestions(params: VisualQuestionRequest): Promise<{
    ...
    try {
      ...
      logger.info(`Generated ${visualQuestions.length} visual questions in ${totalTime}ms`);
  ...
  private static async generateVisualAwareQuestions(params: VisualQuestionRequest): Promise<any[]> {
    ...
    try {
      ...
      logger.warn(`AI generation failed, using enhanced mock questions: ${error}`);
   ...
   */
  private static async generateVisualContent(
    requirement: VisualContentRequirement,
    questionText: string,
    subject: string
    ...
    if (requirement.templateSuggestion) {
      try {
        const templateResult = await ImageGenerationService.generateQuestionImage({
        ...
        logger.warn(`Template generation failed, trying AI: ${templateError}`);
      }
    }
    ...
    return {
      ...
      generationType: aiResult.generationType as any
    };
  }
